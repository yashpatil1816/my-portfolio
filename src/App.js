...
const GlobalStyle = () => (
  <style>{`
    @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600&family=DM+Mono:wght@300;400;500&family=Playfair+Display:wght@600;700&display=swap');
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @media (max-width: 960px) {
      .hero-grid { grid-template-columns: 1fr; gap: 48px; padding: 48px 0 64px; }
      .hero-right { order: -1; }
    }
    ...
    /* JOURNEY */
    .journey-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1px; border: 1px solid var(--line); }
...
const Typewriter = ({ words, speed = 70, pause = 2200 }) => {
  ...
  useEffect(() => {
    ...
    const t = del
      ...
      : sub < word.length
        ? setTimeout(() => setSub(s => s + 1), speed)
        : setTimeout(() => setDel(true), pause);
    return () => clearTimeout(t);
  }, [idx, sub, del, words, speed, pause]);
  return (
    <>
      <span style={{ color: "var(--accent)" }}>{words[idx].slice(0, sub)}</span>
...
const Counter = ({ to, suffix, label }) => {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => { s += step; if (s >= to) { setN(to); clearInterval(t); } else setN(s); }, 18);
    return () => clearInterval(t);
  }, [vis, to]);
  return (
    <div ref={ref} style={{ textAlign: "center", padding: "28px 20px", borderRight: "1px solid var(--line)" }}>
      <div style={{ fontFamily: "var(--serif)", fontSize: "clamp(28px,3.5vw,38px)", fontWeight: 700, color: "var(--text)" }}>
        {Number.isInteger(to) ? Math.round(n) : parseFloat(n).toFixed(2)}{suffix}
      </div>
      <div style={{ fontSize: 11, color: "var(--muted)", letterSpacing: "0.1em", marginTop: 6, textTransform: "uppercase", fontFamily: "var(--mono)" }}>{label}</div>
    </div>
  );
};

/* ─────────────────────────────────────────────
   WEATHER WIDGET (for project modal)
───────────────────────────────────────────── */
const WeatherWidget = () => {
  ...
  const fetchWeather = useCallback(async (q) => {
    ...
    try {
      const res = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(q)}&appid=bd5e378503939ddaee76f12ad7a97608&units=metric`);
      if (!res.ok) throw new Error("City not found");
      setData(await res.json());
    } catch (e) { setError(e.message); } finally { setLoading(false); }
  }, []);
  useEffect(() => { fetchWeather("Pune"); }, []);
  const icon = (m = "") => { m = m.toLowerCase(); if (m.includes("clear")) return "☀️"; if (m.includes("cloud")) return "⛅"; if (m.includes("rain")) return "🌧️"; if (m.includes("snow")) return "❄️"; return "🌡️"; };
  return (
    <div style={{ background: "var(--bg2)", border: "1px solid var(--line)", borderRadius: 8, padding: 16, marginTop: 12 }}>
      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && fetchWeather(input)} placeholder="Search city..." className="field" style={{ flex: 1 }} />
        <button onClick={() => fetchWeather(input)} className="btn-primary" style={{ padding: "11px 14px", whiteSpace: "nowrap" }}>Search</button>
      </div>
      {loading && <p style={{ fontSize: 12, color: "var(--muted)" }}>Loading...</p>}
      {error && <p style={{ fontSize: 12, color: "#dc2626" }}>{error}</p>}
      {data && !loading && (
        <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
          <span style={{ fontSize: 32 }}>{icon(data.weather[0]?.main)}</span>
          <div>
            <p style={{ fontWeight: 600, fontSize: 14 }}>{data.name}, {data.sys.country}</p>
            <p style={{ fontFamily: "var(--serif)", fontSize: 26, fontWeight: 700 }}>{Math.round(data.main.temp)}°C</p>
            <p style={{ fontSize: 12, color: "var(--sub)", textTransform: "capitalize" }}>{data.weather[0]?.description} · Humidity {data.main.humidity}%</p>
          </div>
        </div>
      )}
    </div>
...
const PROJECTS = [
  {
    ...
    description: "Production-grade movie ticket booking backend with JWT auth, real-time seat availability, and role-based access control.",
    tech: ["Java", "Spring Boot", "MySQL", "JPA/Hibernate", "JWT", "Spring Security"],
    detail: "Designed and built a modular RESTful backend for movie ticket booking. Enforced stateless JWT authentication and role-based authorization (Admin/User). Built APIs for movie management, show scheduling, seat selection, booking workflows, and real-time seat validation.",
    arch: "Controller – Service – Repository",
    github: "https://github.com/yashpatil1816",
  },
  {
    title: "ShopSphere", emoji: "🛒", tag: "Microservices",
    description: "5-service microservices e-commerce platform with API Gateway, load balancing, JWT RBAC, and event-driven messaging.",
    tech: ["Java", "Spring Boot", "Spring Cloud", "JWT", "API Gateway", "Kafka"],
    detail: "Engineered a production microservices platform: user, product, order, payment, notification services — each with its own isolated database. API Gateway for routing and load balancing. Event-driven cross-service messaging for eventual consistency.",
    arch: "Microservices + API Gateway",
    github: "https://github.com/yashpatil1816",
  },
  {
    title: "WeatherSphere", emoji: "🌦️", tag: "API Integration",
    ...
    tech: ["Java", "Spring Boot", "REST API", "OpenWeatherMap", "HTTP Client"],
    detail: "Spring Boot service acting as a proxy to OpenWeatherMap REST API. Handles request routing, response transformation, error management, and JSON deserialization — demonstrating real-world third-party API integration patterns.",
    arch: "API Proxy + Transform",
    isWeather: true,
    github: "https://github.com/yashpatil1816",
  },
  {
    title: "J.P. Morgan Simulation", emoji: "🏦", tag: "Virtual Experience",
    description: "Software engineering simulation: Apache Kafka for async messaging, 4+ REST endpoints, JPA entities with H2 database.",
    tech: ["Java", "Spring Boot", "Apache Kafka", "Spring Data JPA", "H2"],
    detail: "Completed J.P. Morgan Chase & Co. Software Engineering Virtual Experience on Forage. Back-end development with Java + Spring Boot. Apache Kafka for async message processing between 2 services. REST controller implementation with Spring Data JPA.",
...
const SKILLS = [
  ...
  { name: "Microservices & Spring Cloud", level: 76 },
  { name: "Apache Kafka", level: 72 },
  { name: "DSA & Problem Solving", level: 78 },
];

const TECHS = [
  "Java", "Spring Boot", "Apache Kafka", "REST API",
  "JPA / Hibernate", "Spring Security", "JWT Auth",
  "MySQL", "Spring Cloud", "Microservices", "Git / Maven", "Postman",
];

const TIMELINE = [
  { year: "2023", title: "B.Tech Begins", desc: "Enrolled at G.H. Raisoni College of Engineering, Pune. Deep-dived into OOP, data structures, and Java." },
  { year: "2024", title: "Spring Boot Mastery", desc: "Solid foundation in Spring Boot, Spring Security, JPA/Hibernate. Solved 100+ DSA problems on LeetCode." },
  { year: "Jan 2025", title: "J.P. Morgan Simulation", desc: "Completed J.P. Morgan Chase Software Engineering Virtual Experience — Kafka, REST APIs, JPA." },
  { year: "Oct 2025", title: "Oracle AI Certified", desc: "Earned Oracle Cloud Infrastructure 2025 AI Foundations Associate certification — ID: 3232586990." },
  { year: "Jan 2026", title: "CineReserve Launched", desc: "Production-grade movie booking backend: JWT auth, RBAC, normalized DB schema, full RESTful API." },
  { year: "2026–27", title: "ShopSphere + Graduation", desc: "Building 5-service microservices platform. B.Tech completion CGPA 8.18/10 — targeting Java Backend roles." },
];
...
export default function App() {
  ...
  // EmailJS integration — sends form to yashpatil2571@gmail.com
  const handleSubmit = async () => {
    ...
    try {
      ...
      await window.emailjs.send("YOUR_SERVICE_ID", "YOUR_TEMPLATE_ID", {
        ...
        to_email: "yashpatil2571@gmail.com",
      });
      setSent(true);
    ...
    <div style={{ background: "var(--bg)", minHeight: "100vh" }}>
      ...
      <motion.div
        style={{
          ...
          background: "var(--accent)", transformOrigin: "left", scaleX: scrollYProgress, zIndex: 1000,
        }}
      />
      ...
      <nav style={{
        ...
        <div className="wrap" style={{ height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div className="nav-links">
            ...
            <a href="https://drive.google.com/file/d/1xboyFm7F2BcgQP7-cH7tG4eqwKYwUfRz/view?usp=drive_link"
          ...
          <button className="nav-ham" onClick={() => setMobileMenu(m => !m)}
            style={{ background: "none", border: "1px solid var(--line)", borderRadius: 6, padding: "7px 10px", cursor: "pointer", display: "flex", flexDirection: "column", gap: 4 }}>
            {[0, 1, 2].map(i => (
              <div key={i} style={{
                width: 18, height: 1.5, background: "var(--text)", borderRadius: 1, transition: "all 0.2s",
                transform: mobileMenu ? (i === 0 ? "rotate(45deg) translate(3.5px,3.5px)" : i === 2 ? "rotate(-45deg) translate(3.5px,-3.5px)" : "scaleX(0)") : "none",
              }} />
            ))}
          </button>
        </div>
        ...
        {mobileMenu && (
          <div style={{ background: "var(--bg)", borderTop: "1px solid var(--line)", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
            {NAV_ITEMS.map(l => (
              <button key={l} onClick={() => scrollTo(l.toLowerCase())}
                style={{ background: "none", border: "none", cursor: "pointer", fontFamily: "var(--sans)", fontSize: 15, fontWeight: 500, color: "var(--sub)", textAlign: "left", padding: 0 }}>
                {l}
              </button>
            ))}
            <a href="https://drive.google.com/file/d/1xboyFm7F2BcgQP7-cH7tG4eqwKYwUfRz/view?usp=drive_link" target="_blank" rel="noreferrer" className="btn-primary" style={{ width: "fit-content" }}>
              ↓ Download Resume
            </a>
          </div>
        )}
      ...
      <section style={{ paddingTop: 64 }}>
        <div className="wrap hero-grid">
          ...
          <div>
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 7, marginBottom: 24 }}>
                ...
                <span style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--green)", letterSpacing: "0.06em" }}>Available for opportunities</span>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }}
              style={{ fontFamily: "var(--mono)", fontSize: "clamp(13px,1.8vw,15px)", color: "var(--sub)", marginBottom: 22, minHeight: 24 }}>
            ...
            <motion.p initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
              style={{ fontSize: 15, color: "var(--sub)", lineHeight: 1.8, maxWidth: 500, marginBottom: 36 }}>
              Building scalable RESTful APIs and microservices with Spring Boot.
              Focused on clean architecture and backend systems that hold up at scale.
              <br />
              <span style={{ fontSize: 12, color: "var(--muted)", marginTop: 8, display: "block", fontFamily: "var(--mono)" }}>
                B.Tech IT · G.H. Raisoni College, Pune · CGPA 8.18/10
              </span>
            </motion.p>
          ...
          <motion.div className="hero-right" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2, duration: 0.6 }}
            ...
            {/* Photo */}
            <div style={{ width: 200, height: 200, borderRadius: "50%", overflow: "hidden", border: "3px solid var(--line)", flexShrink: 0, position: "relative" }}>
              <img src="/profile.jpg" alt="Yash Patil" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 8%" }} />
              <div style={{ position: "absolute", bottom: 12, right: 12, width: 16, height: 16, borderRadius: "50%", background: "var(--green)", border: "3px solid var(--bg)" }} />
            </div>

            {/* Social icons */}
            <div style={{ display: "flex", gap: 10 }}>
              {[
                { label: "GitHub", url: "https://github.com/yashpatil1816", icon: GH_ICON },
                { label: "LinkedIn", url: "https://www.linkedin.com/in/yash-patil-39941a326/", icon: LI_ICON },
                { label: "Email", url: "mailto:yashpatil2571@gmail.com", icon: EM_ICON },
              ].map(c => (
                <a key={c.label} href={c.url} target={c.url.startsWith("http") ? "_blank" : undefined} rel="noreferrer"
                  style={{ width: 36, height: 36, borderRadius: "50%", border: "1px solid var(--line)", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--sub)", transition: "all 0.18s" }}
            ...
            {/* Cert badge */}
            <div style={{ width: "100%", padding: "14px 16px", border: "1px solid var(--line)", borderRadius: 8, background: "#fff", display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ fontSize: 22 }}>🏅</span>
              <div>
                <p style={{ fontFamily: "var(--mono)", fontSize: 9, color: "var(--muted)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 2 }}>Certified</p>
                <p style={{ fontSize: 12, color: "var(--text)", fontWeight: 500, lineHeight: 1.4 }}>Oracle Cloud Infrastructure 2025 AI Foundations</p>
              </div>
            </div>
            ...
            <div style={{ width: "100%", border: "1px solid var(--line)", borderRadius: 8, overflow: "hidden" }}>
              {[
                ...
                { label: "Education", value: "B.Tech IT, 2027" },
                { label: "Status", value: "Open to Work ✓" },
              ].map((row, i) => (
                <div key={row.label} style={{
                  display: "flex", padding: "11px 14px", gap: 12, alignItems: "center",
                  ...
                  <span style={{ fontSize: 12, color: row.label === "Status" ? "var(--green)" : "var(--text)", fontWeight: row.label === "Status" ? 600 : 400 }}>{row.value}</span>
                </div>
              ))}
            </div>
          </motion.div>
      ...
      <section id="about" className="section" style={{ background: "var(--bg2)", borderTop: "1px solid var(--line)", borderBottom: "1px solid var(--line)" }}>
        <div className="wrap">
          ...
          <div className="about-grid">
            <div>
              <p style={{ fontSize: 15, color: "var(--sub)", lineHeight: 1.9, marginBottom: 16 }}>
                Java Backend Developer with hands-on experience building scalable microservices and RESTful APIs with Spring Boot. Proficient in JWT-based auth, third-party API integration, and layered architecture design.
              </p>
              <p style={{ fontSize: 15, color: "var(--sub)", lineHeight: 1.9, marginBottom: 28 }}>
                Strong foundation in relational database modeling, OOP principles, system design, and DSA. Pursuing B.Tech in Information Technology at G.H. Raisoni College of Engineering, Pune — CGPA 8.18/10, Expected 2027.
              </p>
            ...
            <div style={{ border: "1px solid var(--line)", borderRadius: 10, overflow: "hidden", background: "#fff" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
                {[
                  { to: 4, s: "+", l: "Projects Shipped" },
                  { to: 5, s: "", l: "Microservices" },
                  { to: 100, s: "+", l: "DSA Problems" },
                  { to: 8.18, s: "/10", l: "CGPA" },
                ].map((s, i) => (
      ...
      {/* ── PROJECTS ── */}
      <section id="projects" className="section" style={{ background: "var(--bg2)", borderTop: "1px solid var(--line)", borderBottom: "1px solid var(--line)" }}>
        <div className="wrap">
      ...
      {/* ── CONTACT ── */}
      <section id="contact" className="section" style={{ background: "var(--bg2)", borderTop: "1px solid var(--line)" }}>
        <div className="wrap">
          <SectionHeader label="04 — Contact" title="Let's Connect" />
          <p style={{ fontSize: 14, color: "var(--sub)", marginBottom: 44, fontFamily: "var(--sans)", maxWidth: 480 }}>
            Open to Java Backend, SWE & Internship opportunities. Feel free to reach out — messages go directly to my inbox.
          </p>
          ...
          <div className="contact-grid">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {[
                { label: "GitHub", handle: "yashpatil1816", url: "https://github.com/yashpatil1816", icon: GH_ICON },
                { label: "LinkedIn", handle: "yash-patil-39941a326", url: "https://www.linkedin.com/in/yash-patil-39941a326/", icon: LI_ICON },
                { label: "Email", handle: "yashpatil2571@gmail.com", url: "mailto:yashpatil2571@gmail.com", icon: EM_ICON },
                {
                  label: "Phone", handle: "+91 9022046356", url: "tel:+919022046356",
                ...
                <motion.a key={i} href={c.url} target={c.url.startsWith("http") ? "_blank" : undefined} rel="noreferrer"
                  ...
                  style={{
                    ...
                    background: "#fff", border: "1px solid var(--line)", borderRadius: 8, transition: "all 0.18s",
                  }}
            ...
            {/* Contact form */}
            <div style={{ background: "#fff", border: "1px solid var(--line)", borderRadius: 10, padding: "28px 24px" }}>
              ...
              {sent ? (
                ...
                <>
                  ...
                  <p style={{ fontSize: 11, color: "var(--muted)", marginTop: 10, textAlign: "center", fontFamily: "var(--mono)" }}>
                    Goes directly to yashpatil2571@gmail.com
                  </p>
                </>
              )}
            </div>
      ...
      <footer style={{ borderTop: "1px solid var(--line)", padding: "24px 0", background: "var(--bg)" }}>
        <div className="wrap footer-inner">
          ...
          <div style={{ display: "flex", gap: 12 }}>
            {[
              { icon: GH_ICON, url: "https://github.com/yashpatil1816" },
              { icon: LI_ICON, url: "https://www.linkedin.com/in/yash-patil-39941a326/" },
              { icon: EM_ICON, url: "mailto:yashpatil2571@gmail.com" },
            ].map((c, i) => (
      ...
      <AnimatePresence>
        {modal && (
          <motion.div
            ...
            style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(6px)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
            <motion.div
              initial={{ scale: 0.95, opacity: 0, y: 16 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0 }}
              onClick={e => e.stopPropagation()}
              style={{ background: "#fff", border: "1px solid var(--line)", borderRadius: 12, padding: "clamp(20px,4vw,36px)", maxWidth: 580, width: "100%", maxHeight: "85vh", overflowY: "auto", position: "relative" }}>
              <button onClick={() => setModal(null)}
                style={{ position: "absolute", top: 14, right: 14, background: "var(--bg2)", border: "1px solid var(--line)", borderRadius: 6, width: 30, height: 30, cursor: "pointer", color: "var(--sub)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>✕</button>
